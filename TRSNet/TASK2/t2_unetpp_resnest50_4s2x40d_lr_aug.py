...
hr_name = 't2_unetpp_resnest50d_4s2x40d_hr_aug'
...
dataset_train = SegmentationDataSet2(
    inputs=inputs_train, targets=targets_train, n_classes=N_CLASSES, transform=transforms, use_cache=cache_dataset, resize=True, new_size=new_size, augmenter=augmenter
)
...
with open(f"{results_dir}/{lr_name}/metrics.csv", "w", newline="") as outfile:
...
for i in range(10):
    ...
    plt.savefig(f'{results_dir}/{lr_name}/figures/trial_{idx}.png')
    #plt.show()
    plt.close()
