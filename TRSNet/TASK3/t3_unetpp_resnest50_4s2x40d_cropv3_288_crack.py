...
results_dir = "results"
weights_dir = "weights"
model_name = "t3_unetpp_cropv3_aug_crack"
hr_name = 'cropv3_crack_cont2'
hr_save = 'cropv3_crack_cont2'
...
# aug_p={'RandomSizedCrop':0}

data_augmentation = CreateAugmenter(image_height, image_width,)# aug_p_edit=aug_p)

# crop_augmentation = A.CropNonEmptyMaskIfExists(270, 480, ignore_channels=[0,1], p=1.0)
crop_augmentation = A.CropNonEmptyMaskIfExists(270, 480, p=1.0)

augmenter = A.Compose([data_augmentation, crop_augmentation], p=1.0)


random.seed(10)

dataset_train = DataSetTask3CropRbSP(
    inputs=inputs_train, targets=targets_train, n_classes=N_CLASSES, transform=transforms, use_cache=True, resize=False, augmenter=augmenter,#new_size=new_size,
)

# dataset validation
dataset_val = DataSetTask3CropRbSP(
    inputs=inputs_val, targets=targets_val,  n_classes=N_CLASSES, transform=transforms, use_cache=True, resize=False, augmenter=crop_augmentation,
)
...
# print(f'Ignored channels: {dataloader_training.dataset.augmenter.ignore_channels}')
...
loss = smp_edited.losses.focal.FocalLoss(mode='multilabel', alpha=0.6, gamma=2.0,)
loss.__name__ = 'focal_loss'

met1 = smp_edited.utils.metrics.Recall(activation=nn.Sigmoid)

met2 = smp_edited.utils.metrics.IoU(mode='multilabel', from_logits=True,)
...
scheduler = ReduceLROnPlateauPatch(optimizer, factor=0.5, patience=50, verbose=True, threshold=0.0001, min_lr=1E-5)
...
# checkpoint = torch.load(f'{weights_dir}/{hr_name}/{model_name}_checkpoint.pt')
# model.load_state_dict(checkpoint)

# train model

checkpoint = torch.load(f'{weights_dir}/{hr_save}/{model_name}_loss_checkpoint.pt')
model.load_state_dict(checkpoint)
...
for i in range(0, EPOCHS):
    ...
    with open(f'{weights_dir}/{hr_name}/{model_name}_train_logger.csv', 'a', newline='') as f:
        w = csv.writer(f)
        w.writerow(list(train_logs.values()))

    with open(f'{weights_dir}/{hr_name}/{model_name}_valid_logger.csv', 'a', newline='') as f:
        w = csv.writer(f)
