...
from decode_head_S import ResDecoderPU, Segmentor_swin_resd
...
if __name__ == '__main__':   
    ...
    model_bbone = SwinTransformer(num_classes=3,patch_size=2,
                                  embed_dim=48, depths=[2, 2, 2, 6, 2], num_heads=[4, 4, 8, 16, 32])
    ...
    for epoch in range(n_epoch):
        ...
        # metric_epoch_bin_val.append(val_metric_bin)
        ...
        if loss_epoch_bin_val[-1]==np.amin(loss_epoch_bin_val):
            torch.save(model.state_dict(), 'custom_tr.pt')
            patience=0
        else:
            patience+=1
        # Get current lr
